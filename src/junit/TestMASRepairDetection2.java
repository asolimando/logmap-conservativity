...
public class TestMASRepairDetection2 {
	...
	public void test() throws Exception {
		...
		RepairStatus rs = masRepair.assessMapping(manager, actualMappings, 
				mappings.get(id), true, REPAIR_METHOD.SUB, violKinds);
		
		if(rs.getViolationsNumber(violKinds) != 3)
			fail("3 violations should have been detected, got " + 
					rs.getViolationsNumber(violKinds));
		
		// solution not unique
		int repSize = LogMapWrapper.countMappings(rs.getRepairedMappings());
		String javaVersion = System.getProperty("java.specification.version");
//		System.out.println("Java version JVM: " + javaVersion);
		
//		if(javaVersion.equals("1.7") && repSize != 31)
//			fail("Repaired alignment should consist of 31 mappings, found " + repSize);
//		else if(javaVersion.equals("1.8")  && repSize != 30)
//			fail("Repaired alignment should consist of 30 mappings, found " + repSize);
		
				
		if(!LogMapWrapper.isContained(mappings.get(id), rs.getRepairedMappings()))
			fail("Mapping " + mappings.get(id) + " should have not been removed by the repair process");
