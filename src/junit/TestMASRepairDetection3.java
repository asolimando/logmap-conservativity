...
public class TestMASRepairDetection3 {
	...
	public void test() throws Exception {
		...
		// no eq violations!
		Set<MappingObjectStr> actualMappings = new HashSet<>(mappings.subList(0, 7));
				
		RepairStatus rs = masRepair.assessMapping(manager, actualMappings, 
				mappings.get(8), true, REPAIR_METHOD.SUB, violKinds);
				
		if(rs.hasViolations())
			fail("EQ violations should not have been detected");
	
		if(rs.getRepairedMappings().contains(mappings.get(8)))
			fail("Incompatible mapping should have been discarded");
