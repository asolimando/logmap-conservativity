...
public class EvalImpactOAEI_ELK_MORe_2013 {
	...
	private static void EvaluateCoherence2013() throws Exception{
		...
		//reasoner = ReasonerManager.HERMIT; //0
		...
		///home/ernesto/OM_OAEI/REASONING_2013 in elmo
	}
	...
	private static void EvaluateCoherence2013(
		...
		for(int i=0; i<filenames.length; i++){
			...
			System.out.println("Unsatisfiable clases using ELK: " + unsat);
			
			fin = Calendar.getInstance().getTimeInMillis();
			System.out.println("Done ELK, Time (s): " + (float)((double)fin-(double)init)/1000.0);
			...
			if (reasoner == ReasonerManager.HERMIT){
				reasonerAccess = new HermiTAccess(manager, onto_merged, false);
				
				try{
					reasonerAccess.classifyOntology_withTimeout_throws_Exception(7200);
					
					unsat = reasonerAccess.getUnsatisfiableClasses().size();
					
					System.out.println("Unsatisfiable clases using HermiT: " + unsat);
					
					fin = Calendar.getInstance().getTimeInMillis();
					System.out.println("Done HermiT reasoner, Time (s): " + (float)((double)fin-(double)init)/1000.0);
					
				}
				catch (Exception e){
					timeout=true;
					System.out.println("Timeout using HermiT: 7200s");
				}
				
			}
			else if (reasoner == ReasonerManager.MORe) {
				...
				try{
					...
					System.out.println("Unsatisfiable clases using MOre: " + unsat);
					
					fin = Calendar.getInstance().getTimeInMillis();
					System.out.println("Done MORe reasoner, Time (s): " + (float)((double)fin-(double)init)/1000.0);
					
				}
				catch (Exception e){
					timeout=true;
					System.out.println("Timeout using MORe: 7200s");
				}
			...
			if (timeout)
				System.out.println("Results\t: " + num_mappings + "\t" + unsat + "\t" + timeout);
			else
				System.out.println("Results\t: " + num_mappings + "\t" + unsat);
			
			
			
			
			manager.removeOntology(onto_merged);
			System.out.println("\n\n");				
	...
	private static OWLOntology getOWLOntologyFromRDFMappings(String file) throws Exception{
		...
		num_mappings = managerReader.getMappingObjectsSize();
		
		LogOutput.print("Read RDF Align mapping objects: " + managerReader.getMappingObjectsSize() + " - " + num_mappings);
	...
	public static void main(String[] args) {
		...
		try{
			if (args.length==5){
				 EvaluateCoherence2013(
						 args[0], //path_base_onto
						 args[1], //path_base_mappings
						 Integer.valueOf(args[2]), //int ontoPair, 
						 args[3], //sizePair 
						 Integer.valueOf(args[4])); //int reasoner)
			}
			else{
				EvaluateCoherence2013();
			}
