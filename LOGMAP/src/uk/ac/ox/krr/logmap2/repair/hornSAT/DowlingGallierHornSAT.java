...
public class DowlingGallierHornSAT {
	...
	public DowlingGallierHornSAT(DowlingGallierHornSAT dg){
		
		this.clause_num = dg.clause_num;
		this.R = new HashSet<>(dg.R);
		this.Q = new Stack<Integer>();
		Q.addAll(dg.Q);
		this.N = new Vector<Integer>(dg.N);
//		this.generalIgnoreFS = new HashMap<>();
//		this.additionalIgnoreFS = new HashMap<>();
//		this.FS = new HashMap<>();
		
		for (Entry<Integer, Set<Link>> e : dg.generalIgnoreFS.entrySet())
			this.generalIgnoreFS.put(e.getKey(), new HashSet<>(e.getValue()));
		
		for (Entry<Integer, Set<Link>> e :dg.additionalIgnoreFS.entrySet())
			this.additionalIgnoreFS.put(e.getKey(), new HashSet<>(e.getValue()));
		
		for (Entry<Integer, Set<Link>> e :dg.FS.entrySet())
			this.FS.put(e.getKey(), new HashSet<>(e.getValue()));
		
		for (Entry<Integer, HornClause> e :dg.clauses.entrySet())
			this.clauses.put(e.getKey(), e.getValue());
		
		conflictive_disjointness  = new HashSet<HornClause>(dg.conflictive_disjointness);
		conflictive_mappings = new HashSet<HornClause>(dg.conflictive_mappings);
		mappings_involved_in_error = new HashSet<HornClause>(dg.mappings_involved_in_error);
		disjointness_involved_in_error = new HashSet<HornClause>(dg.disjointness_involved_in_error);
	}
