function matrix2latex(matrix, matrix2, filename, varargin)
...
%    end
	...
	matrix2 = num2cell(matrix2);
		...
		if w > width2
			if(~isempty(format))
		            matrix{h, w} = num2str(matrix{h, w}, format);%'%-6.3f');
		        else
    ...
    for h=1:height
		...
		fprintf(fid, '%s&', strcat(matrix{h, w}, '(', matrix2{h,w}, ')'));
	    end            
        end
        w = max([1;w+1]);
	if w > width2
