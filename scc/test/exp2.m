function exp2(infolder,outfolder,outFile,pattern,aggrCol,aggrColExt)
...
for d = 1:length(dirListing)
    if ~dirListing(d).isdir
        ...
        resultsStd = [];
        rowLabels = [];
        colLabels = {'\% D/PM','\% PM/M','\% D/M','\% 1-1M','$\vert$ M $\vert$',...
            '\% VtxSCC/Vtx','probSCC','NontrGlSCC','ASP (s)','\% OptDiag','\#M',...
            '\#TotM'};
        percCols = [(1 : 4), 6, 10];
